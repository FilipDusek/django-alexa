...
ALEXA_APP_IDS = dict([(str(os.environ[envvar]), envvar.replace("ALEXA_APP_ID_", "")) for envvar in list(os.environ.keys()) if envvar.startswith('ALEXA_APP_ID_')])
...
def validate_app_ids(value):
    ...
    """
    if value not in list(ALEXA_APP_IDS.keys()):
